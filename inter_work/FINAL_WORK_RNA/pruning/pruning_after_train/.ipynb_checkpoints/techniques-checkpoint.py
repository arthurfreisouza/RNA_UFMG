...
def train_ELM_PRUNING(xin : np.ndarray, yin : np.ndarray, p : int, keep_rate : float, control : bool) -> list:
    ...
    '''if control == True:
        ...
        for i in range(p):
            ...
            np.random.seed(np.random.randint(0, 10000))
            col = 10*np.random.random(n + 1) + 15*np.random.uniform(low = 0, high = 5, size = n + 1)
            Z[: , i ] = random_num*col
        ones = np.ones((xin.shape[0], 1))
        xin = np.concatenate((xin,ones), axis = 1)
    else:
        for i in range(p):
            ...
            random_num = np.random.rand()
            np.random.seed(np.random.randint(0, 10000))
            col = 10*np.random.random(n) + 15*np.random.uniform(low = 0, high = 5, size = n)
            Z[: , i ] = random_num*col'''
